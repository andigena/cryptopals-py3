...
# CAUTION calling update multiple times behaves incorrectly (pads after every invocation)
class SHA1:
    ...
    @staticmethod
    def padding(stream, prefix_len=0):
        l = len(stream) + prefix_len  # Bytes
        ...
        # print('Padded: {}: {}'.format(len(stream), stream))
        return stream
    ...
    def update(self, stream, prefix_len=0):
        stream = SHA1.padding(stream, prefix_len)
...
def authenticate(key, msg, mac):
    calculated_mac, _ = mac_sha1(key, msg)
